...
case class OpContext(vars: Map[String, (String, Any)], funs: Map[String, (String, Any)]) {
	...
	def getFunction[A: Type, B: Type](name: String): OpContext.SearchResult[A => Expr[B]] =
		funs.get(name) match
			case Some((t, f)) if t == Type[A => Expr[B]].name => OpContext.SearchResult.Found(f.asInstanceOf[A => Expr[B]])
			case Some((t, _)) => OpContext.SearchResult.TypeMismatch(Type[A => Expr[B]].name, t)
			case None => OpContext.SearchResult.NotFound
	
	def addVariable[A: Type](name: String, value: A): OpContext =
		copy(vars = vars.updated(name, Type[A].name -> value))

	def addFunction[A: Type, B: Type](name: String, fn: A => Expr[B]): OpContext =
		copy(funs = funs.updated(name, Type[A => Expr[B]].name -> fn))
}
